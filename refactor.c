...
// 64 byte block of memory accessed with different types
typedef union {
...
{
    //TODO: Can adopt method from this for MD5
...
{
    // Expect and open a single file name
    if (argc != 2)
    {
    ...
    // nextblock params
    BLOCK M;
    uint64_t nobits = 0; // number of bits read in 64 bit integer
    PADFLAG status = READ;
    ...
    while (nextblock(&M, infile, &nobits, &status))
    {
        ...
        nexthash(M.threetwo, H); // Passing memory address of first 32 bit intger
    }
